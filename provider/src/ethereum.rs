...
/// Fetch the latest Ethereum block header using a raw JSON-RPC POST.
/// Returns Ok(Some(BlockHeader)) when a block is found, Ok(None) when result is null.
pub async fn fetch_latest_block(client: &Client, endpoint: &str) -> Result<Option<BlockHeader>> {
    let body = serde_json::json!({
        "jsonrpc": "2.0",
        "method": "eth_getBlockByNumber",
        "params": ["latest", false],
